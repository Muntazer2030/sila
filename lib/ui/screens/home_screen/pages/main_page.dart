...
class _MainPageState extends State<MainPage> {
  ...
  // لحفظ أرقام الفهرس (Index) للفئات التي يجب عرضها في البطاقات العلوية
  ...
  Future<void> _loadData() async {
    ...
    final allCampaigns = await DatabaseHelper.instance.getAllCampaigns();
    ...
    for (int i = 1; i < widget.appData.categories.length; i++) {
      String catName = widget.appData.categories[i]["name"];

      if (catName.contains("إعدادات") || catName.contains("اعدادات")) continue;

      validIndices.add(i);

      if (catName.contains("حملات") || catName.contains("فعاليات")) {
        widget.appData.categories[i]["count"] = allCampaigns.length;
      } else {
  ...
  Future<void> _openAddScreen(String categoryName) async {
    ...
    bool? added = await Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) =>
            ProfileEditScreen(initialData: newProfile, isEditMode: false),
      ),
    );
                      ...
                      children: [
                              ...
                              Expanded(
                                ...
                                child: GridView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    int catIndex = _displayIndices[index];
                        ...
                        GridView.count(
                          ...
                          children: [
                            ...
                            _buildActionCard(
                              "اضافة شخصية أكاديمية",
                              Icons.school_outlined,
                              c1,
                              "الشخصيات الأكاديمية",
                            ),
                            _buildActionCard(
                              "اضافة شخصية عامة أو اجتماعية",
  ...
  Widget _buildActionCard(
    ...
    return GestureDetector(
      ...
      child: MainPageAddingCard(title: title, icon: icon, color: color),
    );
  }
}
